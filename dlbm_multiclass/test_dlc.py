"""
    ...
    It is used to test DLC1 and DLC8.
...
outputs = 3
display_labels = ['diff2', 'diff3', 'feedback']
...
accuracy = torchmetrics.classification.Accuracy(task='multiclass', average='macro', num_classes=outputs)
precision = torchmetrics.classification.Precision(task="multiclass", average='macro', num_classes=outputs)
recall = torchmetrics.classification.Recall(task="multiclass", average='macro', num_classes=outputs)
f1_score = torchmetrics.classification.F1Score(task="multiclass", average='macro', num_classes=outputs)
conf_mat = torchmetrics.classification.ConfusionMatrix(task="multiclass", num_classes=outputs)
...
disp.plot()
disp.figure_.savefig(f"plots/dlc/DLC8.pdf", bbox_inches="tight")

fig, ax = plt.subplots(figsize=(10, 10))
disp = ConfusionMatrixDisplay(confusion_matrix=conf_matrix_majority,
                              display_labels=display_labels)
disp.plot()
disp.figure_.savefig(f"plots/dlc/DLC1.pdf", bbox_inches="tight")